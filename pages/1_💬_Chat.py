...
def save_history():
    if r:
        r.setex(redis_key, 86400 * 7, json.dumps(st.session_state.messages)) # Save for 7 days
            ...
            with st.expander("View Sources"):
                for idx, src in enumerate(message["sources"]):
                    ...
                    if content_type == "youtube" and src.get("timestamp_url"):
                        ...
                        if ts_label:
                            st.markdown(f"**Source {idx + 1}** 📺 [{display_title} @ {ts_label}]({src['timestamp_url']})")
                        else:
                            st.markdown(f"**Source {idx + 1}** 📺 [{display_title}]({src['timestamp_url']})")
                    else:
