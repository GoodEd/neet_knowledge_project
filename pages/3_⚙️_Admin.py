...
if not st.session_state.admin_logged_in:
    ...
    # We use a simple env var for password. Fallback is "admin123" for dev.
...
def get_rag_system():
    ...
    return NEETRAG(llm_provider=llm_provider, llm_model=llm_model, llm_base_url=base_url)
...
st.markdown("Use this panel to ingest and manage documents and videos in the RAG vector store.")
...
with col2:
    ...
    for s in sources:
        status_icon = "🟢" if s.status == "active" else "🔴" if s.status == "error" else "⚪"
